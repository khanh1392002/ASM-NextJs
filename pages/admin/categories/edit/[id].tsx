...
=======
                    <input  type="text" id="first-name" autoComplete='given-name' className="mt-4 p-[10px] focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" {...register('name',{required:true, minLength:3})} />
                    {Object.keys(errors).length !== 0 && (
                        <ul>
                            {errors.name?.type == 'required' && (<li className='text-[red] p-[15px]'>Không được để trống</li>)}
                            {errors.name?.type == 'minLength' && <p className='text-[red] p-[15px]'>Phải lớn hơn 3 ký tự</p>}
                        </ul>
                     )}
>>>>>>> 47f72186572dd2900597971c2aa6adfdae09d7ea
              </div>
